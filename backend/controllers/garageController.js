...
const { sendGarageWelcomeEmail } = require('../services/emailService');
...
exports.createGarage = async (req, res) => {
    try {
        ...
        // Send Welcome Email if ownerEmail is provided via centralized email service
        if (req.body.ownerEmail) {
            sendGarageWelcomeEmail({
                ...
                console.error("Failed to send Welcome Email to Garage:", error);
            });
        }
