...
const SortButton = () => {
    ...
    const hasActiveSort = filterConfig && (currentSortOrder !== 'latest' || currentTimeRange !== 'all');
    ...
    return (
        <>
            ...
            <button
                ...
                className={`fixed bottom-[13.95rem] right-9 z-50 p-3 rounded-full border flex items-center justify-center transition-all shadow-sm hover:shadow-md cursor-pointer hover:scale-105 active:scale-95 duration-300 group ${
                    isSortOpen
            ...
            {isSortOpen && (
                <div
                    ref={popupRef}
                    className="fixed bottom-[13.95rem] right-24 z-50 w-max max-w-[calc(100vw-7.5rem)] bg-white/95 backdrop-blur-2xl border border-blue-100/80 rounded-3xl shadow-[0_12px_40px_rgba(0,0,0,0.12)] overflow-hidden p-5 space-y-4 animate-in fade-in zoom-in-95 duration-200"
                >
                    {/* Header */}
                    ...
                    {/* Footer */}
                    {filterConfig && resultsCount !== null && (
