...
const UploadDocuments = () => {
    ...
    const documentTypes = [
        { key: 'panCard', label: 'PAN CARD', subtext: 'KINDLY UPLOAD THE CLEAR IMAGE OF THE DOCUMNENT' },
        { key: 'signature', label: 'SIGNATURE', subtext: 'KINDLY UPLOAD THE CLEAR IMAGE OF THE SIGNATURE' },
        { key: 'voterId', label: 'VOTER CARD', subtext: 'KINDLY UPLOAD THE CLEAR IMAGE OF THE DOCUMNENT' },
        { key: 'adharCard', label: 'AADHAR CARD', subtext: 'KINDLY UPLOAD THE CLEAR IMAGE OF THE DOCUMNENT' },
        { key: 'drivingLicense', label: 'DRIVING LICENSE', subtext: 'KINDLY UPLOAD THE CLEAR IMAGE OF THE DOCUMNENT' },
        { key: 'agreement', label: 'EMPLOYMENT AGREEMENT', subtext: 'KINDLY UPLOAD THE CLEAR IMAGE OF THE DOCUMNENT' },
    ];
    ...
    const getOrGenerateDocId = (empObj, docKey) => {
        if (empObj[`${docKey}DocId`]) {
            return empObj[`${docKey}DocId`].toUpperCase();
        }
        let hash = 0;
        const str = `${empObj._id || empObj.id || ''}-${docKey}`;
        for (let i = 0; i < str.length; i++) {
            hash = (hash * 31 + str.charCodeAt(i)) % 9000000;
        }
        const num = 1000000 + Math.abs(hash);
        const baseId = `D${num}`;
        const wasReuploaded = localStorage.getItem(`doc_reuploaded_${empObj._id || empObj.id}_${docKey}`) === 'true';
    ...
    const handleFileUpload = async (docKey, file) => {
        ...
        try {
            ...
            const currentSt = (localStorage.getItem(`doc_status_${empId}_${docKey}`) || employee[`${docKey}Status`] || '').toUpperCase();
            if (currentSt === 'REJECTED') {
                localStorage.setItem(`doc_reuploaded_${empId}_${docKey}`, 'true');
            }
            ...
            if (res.ok && data.success) {
                ...
                setEmployee(data.data);
                const docId = getOrGenerateDocId(employee, docKey);
                if (employee._id) localStorage.removeItem(`doc_status_${employee._id}_${docKey}`);
                if (employee.id) localStorage.removeItem(`doc_status_${employee.id}_${docKey}`);
                if (employee.employeeId) localStorage.removeItem(`doc_status_${employee.employeeId}_${docKey}`);
                ...
                <table className="w-full text-center border-collapse table-fixed">
                    ...
                    <tbody className="divide-y divide-[#e6f0fa] uppercase text-[12px]">
                        {documentTypes.map((doc) => {
                            ...
                            const uploadedAt = employee[`${doc.key}UploadedAt`];

                            const docId = getOrGenerateDocId(employee, doc.key);
                            const backendStatus = employee[`${doc.key}Status`];
                            const storedStatus = localStorage.getItem(`doc_status_${employee._id}_${doc.key}`) ||
                                                 localStorage.getItem(`doc_status_${employee.id}_${doc.key}`) ||
                                                 localStorage.getItem(`doc_status_${employee.employeeId}_${doc.key}`) ||
                                                     ...
                                                     <button
                                                         ...
                                                         onClick={() => {
                                                             setSelectedDoc({
                                                                 ...
                                                                 fileUrl: fileUrl,
                                                                 remark: employee[`${doc.key}Remark`] || localStorage.getItem(`doc_remark_${docId}`) || ''
                                                             });
                                    ...
                                    {selectedDoc.fileUrl && selectedDoc.fileUrl.match(/\.(jpg|jpeg|png|webp|gif)($|\?)/i) ? (
                                        ...
                                        <img
                                            ...
                                            onError={(e) => {
                                                ...
                                                const fallback = document.getElementById(`pdf-iframe-emp-${selectedDoc.documentId}`);
                                                if (fallback) fallback.style.display = 'block';
                                            }}
                                        />
                                    ) : (
                                        <iframe
                                            id={`pdf-iframe-emp-${selectedDoc.documentId}`}
