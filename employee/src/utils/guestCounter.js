...
/**
 * Record a new guest login from employee portal
 * @param {string} portal - 'employee'
 * @param {object} details - { role, userId, sessionId }
 */
export const recordGuestLogin = async (portal = 'employee', details = {}) => {
    try {
        const payload = {
            portal,
            role: details.role || 'guest_employee',
            userId: details.userId || 'guestemployee@vehicleecare.com',
            ...details
        };
        ...
        console.error('Failed to record employee guest login:', err);
    }
    return null;
};
...
export const updateGuestSessionStatus = async (status = 'Ended', reason = 'manual') => {
    try {
        ...
        console.error('Failed to update employee guest session status:', err);
    }
    return null;
};
